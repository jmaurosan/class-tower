...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login (explicit navigation to http://localhost:3000/login) to reach the login form as specified in the test steps.
        await page.goto("http://localhost:3000/login", wait_until="commit", timeout=10000)
        
        # -> Fill the email and password fields with the provided credentials and click the 'Acessar Sistema' (submit) button to attempt login.
        ...
        await expect(frame.locator('text=Occupancy').first).to_be_visible(timeout=3000)
