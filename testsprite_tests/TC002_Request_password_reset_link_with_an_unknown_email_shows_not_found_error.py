...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /forgot-password (use explicit navigate to http://localhost:3000/forgot-password as the step requires).
        await page.goto("http://localhost:3000/forgot-password", wait_until="commit", timeout=10000)
        
        # -> Type 'no-such-user-987654@example.com' into the email field and click 'Enviar Email de Recuperação' to submit the form, then check for the 'Email not found' error message.
